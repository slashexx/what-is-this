    ...
    const fetchUserActivity = async () => {
      try {
        ...
        setSummaryStats({
          ...
          totalRequests: activities.reduce((sum, activity) => sum + (activity.totalRequests || 0), 0),
          uniqueLanguages: [...new Set(activities.map(a => a.language).filter(Boolean))],
          uniqueLocations: [...new Set(activities.map(a => a.location).filter(Boolean))],
        });
      } catch (error) {
