...
export default function OrdersPage() {
  ...
  const fetchVendors = async () => {
    try {
      const response = await fetch("/api/vendor/getEcommerceVendors");
      if (!response.ok) {
  ...
  return (
    <>
    {detailsViewActive ? (<VendorDetails selectedVendor={selectedVendor} handleBackToList={handleBackToList}/>) : (
      <div className="lg:px-6 md:px-4 lg:py-6 py-2 px-0">
        <div className="flex justify-between items-center mb-6 mt-4 md:mt-0 ">
          <h1 className="sm:text-4xl text-2xl font-bold mr-2 text-[#4D413E]">Vendors</h1>
